...
echo
echo "[step] ad-hoc re-signing bundle (replaces linker-signed sig with proper adhoc)"
codesign --force --deep --sign - "$APP"
chmod -R +x "$APP/Contents/MacOS/"

echo "[step] packaging DMG with /Applications symlink"
DMG="$BUILD/ClaudeGame-mac.dmg"
STAGE=$(mktemp -d)
ditto "$APP" "$STAGE/ClaudeGame.app"
ln -s /Applications "$STAGE/Applications"
rm -f "$DMG"
hdiutil create -volname "ClaudeGame" -srcfolder "$STAGE" -ov -format UDZO -fs HFS+ "$DMG" >/dev/null
rm -rf "$STAGE"
